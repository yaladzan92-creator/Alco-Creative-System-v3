...
async function startServer() {
  ...
  app.post("/api/ai/generate", authenticate, async (req: any, res: any) => {
    try {
      ...
      // Resolve Gemini API key: 1. Request headers, 2. Stored user config
      let apiKey = req.headers["x-gemini-api-key"] || req.headers["X-Gemini-API-Key"] || req.headers["x-api-key"];
      
      if (!apiKey || typeof apiKey !== "string" || apiKey.trim().length === 0) {
        const storedConfig = adminConfigStore[userId];
        if (storedConfig?.geminiApiKey) {
      ...
      if (!apiKey || typeof apiKey !== "string" || apiKey.trim().length === 0) {
        return res.status(403).json({
          error: "API_KEY_REQUIRED",
          message: "Akses AI Ditolak. Anda wajib menyediakan Gemini API Key Anda sendiri di pengaturan atau panel onboarding."
        });
      }
  ...
  app.all("/api/*", (req: any, res: any) => {
    res.status(404).json({
      error: "NOT_FOUND",
      message: `API endpoint ${req.method} ${req.path} tidak ditemukan.`
    });
  });
