...
export default function BrandFoundationStep({ project, onSave, onSaveProject }: BrandFoundationStepProps) {
  ...
  const handleGenerateAIRecommendations = async (customRevision?: string) => {
    ...
    try {
      ...
      const previousOutput = aiAnalysis || project?.brandFoundationData?.aiAnalysis;

      const previousStepsContext = buildRevisionPromptContext({
        revision: customRevision,
        previousOutput,
        stepName: "Step 9: Brand Foundation & Identity",
        defaultContext: `
        Proyek: ${project?.name}
...
Your output MUST be a valid JSON object in Indonesian with these keys:
{
  ...
  return (
    <div className="space-y-8">
      ...
      <StepAiDraftBar
        stepNumber={9}
        stepName="Brand Foundation & Visual Identity"
        project={project}
        currentValues={{
          brandName: formData.brandName,
          companyName: formData.companyName,
        ...
        fieldLabels={{
          ...
          companyName: "Nama Perusahaan / Legalitas (Opsional)",
          collaborations: "Rencana Kolaborasi / Program Afiliasi",
          advertiserFigure: "Sosok Iklan / Endorsement (UGC)",
          brandFeel: "Sentuhan Emosional & Vibe Brand"
        }}
        onApplyAll={(newVals) => {
          setFormData(prev => ({
            ...prev,
            brandName: newVals.brandName || prev.brandName,
            companyName: newVals.companyName || prev.companyName,
            collaborations: newVals.collaborations || prev.collaborations,
            advertiserFigure: newVals.advertiserFigure || prev.advertiserFigure,
            brandFeel: newVals.brandFeel || prev.brandFeel
          }));
        }}
      />
