...
class MockAuth {
  ...
  get currentUser(): User {
    let profileStr = localStorage.getItem("alco_user_profile");
    if (!profileStr) {
      const defaultProfile = { name: "Pengguna ALCO", brandName: "" };
      try {
        localStorage.setItem("alco_user_profile", JSON.stringify(defaultProfile));
        profileStr = JSON.stringify(defaultProfile);
      } catch (_) {}
    }

    let name = "Pengguna ALCO";
    let brandName = "";
    ...
    if (profileStr) {
      try {
        ...
        if (profile.brandName !== undefined) brandName = profile.brandName;
      } catch (_) {}
    }

    if (this.lastProfileStr === profileStr && this.cachedUser) {
...
let memoryDbFallback: Record<string, any> | null = null;

function pruneStorageData(data: Record<string, any>): Record<string, any> {
  const pruned = JSON.parse(JSON.stringify(data));
  
  // 1. Clean out large base64 strings or huge data URIs embedded in project state
  const recursiveClean = (obj: any) => {
    if (!obj || typeof obj !== "object") return;
    for (const key of Object.keys(obj)) {
      if (typeof obj[key] === "string") {
        if (obj[key].startsWith("data:image/") || obj[key].startsWith("data:audio/") || obj[key].length > 250000) {
          obj[key] = "[base64_truncated_for_storage]";
        }
  ...
  recursiveClean(pruned);

  // 2. Keep top 15 most recent projects if project count exceeds 15
  const projectKeys = Object.keys(pruned).filter(k => k.startsWith("projects/"));
  if (projectKeys.length > 15) {
    projectKeys.sort((a, b) => {
      const timeA = new Date(pruned[a]?.updatedAt || pruned[a]?.createdAt || 0).getTime();
      const timeB = new Date(pruned[b]?.updatedAt || pruned[b]?.createdAt || 0).getTime();
      return timeB - timeA;
    });
    projectKeys.slice(15).forEach(k => delete pruned[k]);
  }
...
function getLocalDB(): Record<string, any> {
  try {
    ...
    let parsed: Record<string, any> | null = data ? JSON.parse(data) : null;
    
    if (!parsed && memoryDbFallback) {
      parsed = memoryDbFallback;
    }
    
    if (!parsed) {
      try {
        const sessionData = sessionStorage.getItem("alco_local_db_backup");
        if (sessionData) parsed = JSON.parse(sessionData);
      } catch (_) {}
    }
    ...
    console.warn("Local DB read error, using fallback state:", error);
    return memoryDbFallback || {};
  }
}

function saveLocalDB(data: Record<string, any>) {
  memoryDbFallback = data;
  try {
    localStorage.setItem("alco_local_db", JSON.stringify(data));
  } catch (error) {
    console.warn("Local DB quota reached, attempting storage cleanup and pruning...", error);
    try {
      const cleaned = pruneStorageData(data);
      localStorage.setItem("alco_local_db", JSON.stringify(cleaned));
      console.log("Local DB storage pruned and saved successfully.");
    } catch (err2) {
      console.warn("Second save attempt failed, trimming to top 5 projects...", err2);
      try {
        const minimal = { ...data };
        const projectKeys = Object.keys(minimal).filter(k => k.startsWith("projects/"));
        if (projectKeys.length > 5) {
          projectKeys.sort((a, b) => {
            const timeA = new Date(minimal[a]?.updatedAt || 0).getTime();
            const timeB = new Date(minimal[b]?.updatedAt || 0).getTime();
            return timeB - timeA;
          });
          projectKeys.slice(5).forEach(k => delete minimal[k]);
        }
        localStorage.setItem("alco_local_db", JSON.stringify(minimal));
      } catch (err3) {
        console.warn("Storage quota limit reached. Retaining state in memory fallback:", err3);
        try {
          sessionStorage.setItem("alco_local_db_backup", JSON.stringify(data));
        } catch (_) {}
      }
    }
  }
