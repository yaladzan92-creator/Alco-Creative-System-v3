...
export function mergeWorkflowResult(project: any): BrandIntelligence {
  ...
  if (project?.initialProductData) {
    const ip = project.initialProductData;
    bi.brandIdentity.brandName = bi.brandIdentity.brandName || ip.productName || "";
    bi.brandIdentity.niche = project?.nicheData?.selectedOption?.name || ip.productName || bi.brandIdentity.niche;
    bi.brandIdentity.industry = ip.productCategory === 'non_digital' ? 'Non-Digital / Retail / Jasa' : 'Digital Products';
    bi.brandIdentity.subNiche = ip.productFormat || bi.brandIdentity.subNiche;
    bi.brandIdentity.usp = ip.mainBenefit || bi.brandIdentity.usp;
  }
  ...
  if (project?.copyDirection) {
    ...
    if (selectedCopy) {
      const safeFormat = (val: any): string => {
        ...
        if (typeof val === "object") {
          if (val.pembuka || val.penjelasan || val.ajakan_beli) {
            const parts = [];
            if (val.pembuka) parts.push(`Pembuka: ${typeof val.pembuka === 'object' ? JSON.stringify(val.pembuka) : val.pembuka}`);
            if (val.penjelasan) parts.push(`Penjelasan: ${typeof val.penjelasan === 'object' ? JSON.stringify(val.penjelasan) : val.penjelasan}`);
            if (val.ajakan_beli) parts.push(`Ajakan Beli: ${typeof val.ajakan_beli === 'object' ? JSON.stringify(val.ajakan_beli) : val.ajakan_beli}`);
            return parts.join(" • ");
          }
      ...
      bi.messaging.toneOfVoice = {
        ...bi.messaging.toneOfVoice,
        tone: safeFormat(selectedCopy.tone),
        style: safeFormat(selectedCopy.style),
        structure_analysis: safeFormat(selectedCopy.structure_analysis),
      };
      
      bi.messaging.copyGuidelines = Array.from(new Set([
        ...bi.messaging.copyGuidelines,
        safeFormat(selectedCopy.summary),
        `Structure: ${safeFormat(selectedCopy.structure_analysis)}`
      ].filter(Boolean)));
    }
  }
