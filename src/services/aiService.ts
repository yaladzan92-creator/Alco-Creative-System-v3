...
export function safeParseJSON(text: string, defaultValue: any = {}): any {
  ...
  // Extract JSON payload bounded by first '{' or '[' and last '}' or ']' if extra text/markdown exists
  ...
  if (startIdx !== -1) {
    ...
    const lastEndIdx = isArray ? cleaned.lastIndexOf(']') : cleaned.lastIndexOf('}');
    if (lastEndIdx > startIdx) {
      cleaned = cleaned.slice(startIdx, lastEndIdx + 1);
    } else {
...
let activeModalHandler: ApiKeyModalHandler | null = null;

export function setApiKeyModalResolver(handler: ApiKeyModalHandler) {
  activeModalHandler = handler;
  return () => {
    if (activeModalHandler === handler) {
      activeModalHandler = null;
    }
  };
}

export function promptApiKey(isInvalid?: boolean): Promise<string> {
  return new Promise((resolve, reject) => {
    if (activeModalHandler) {
      activeModalHandler.open(resolve, reject, isInvalid);
      return;
    }

    // Fallback via Window CustomEvent
    const event = new CustomEvent("alco_open_api_key_modal", {
      detail: { resolve, reject, isInvalid }
    });
    window.dispatchEvent(event);
  });
}

export async function generateAIContent(prompt: string, systemInstruction?: string): Promise<any> {
  const token = (await auth.currentUser?.getIdToken()) || "local-mock-token";
  let personalKey = typeof window !== "undefined" ? localStorage.getItem("alco_gemini_api_key") : null;
  
  // If API key is missing, prompt contextually before attempting call
  if (!personalKey || personalKey.trim().length === 0) {
    try {
      personalKey = await promptApiKey(false);
    } catch (err: any) {
      throw new Error("Aktivasi API Key dibatalkan. Fitur AI membutuhkan Gemini API Key aktif.");
    }
  }
  ...
  if (personalKey && personalKey.trim().length > 0) {
    headers["X-Gemini-API-Key"] = personalKey.trim();
  }
  ...
  if (!response.ok) {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (errorData) {
        ...
        if (response.status === 403 && (errorData.error === "AI_REQUIRED" || errorData.error === "API_KEY_REQUIRED" || errorData.error === "API_KEY_INVALID")) {
          // If server rejects key, open contextual modal and auto-retry on new valid key
          try {
            await promptApiKey(true);
            return await generateAIContent(prompt, systemInstruction);
          } catch (retryErr) {
            throw new Error("Aktivasi API Key dibatalkan.");
          }
        }
  ...
  if (!contentType || !contentType.includes("application/json")) {
    ...
    // Fallback: If returned string is raw text, wrap in text property
  ...
  try {
    localStorage.setItem("alco_has_used_ai", "true");
    if (typeof window !== "undefined") {
      window.dispatchEvent(new Event("alco_ai_used"));
    }
  } catch (_) {}
  return data;
}
...
export const AGENT_PROMPTS = {
  ...
  STEP_6_OFFER: "Rancang 3 variasi paket penawaran (offer stack) berkonversi tinggi yang sulit ditolak pembeli. Setiap opsi WAJIB memiliki 3 bagian inti: (1) mainOffers: [{ name, description, price: number }], (2) bonuses: [{ name, description, value: number }], (3) guarantees: [{ title, duration, description }], dan (4) pricing: { normalPrice: number, discountType: 'percentage' | 'fixed', discountValue: number, finalPrice: number, urgencyNote: string }. Pastikan harga realistis dalam nominal Rupiah. Gunakan Bahasa Indonesia yang persuasif dan mudah dipahami. Kembalikan JSON: { options: [{ id, type, main_offer, mainOffers: [{ name, description, price }], bonuses: [{ name, description, value }], guarantees: [{ title, duration, description }], pricing: { normalPrice, discountType, discountValue, finalPrice, urgencyNote }, pricing_strategy, guarantee, urgency, summary }] }.",
